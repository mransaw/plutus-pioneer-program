...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game)   -> if c1 == c2 then Just ( Constraints.mustBeSignedBy (gFirst game)                  <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)                     <>
                                                       Constraints.mustPayToPubKey (gSecond game) (lovelaceValueOf $ gStake game)           
                                                     , State Finished mempty
                                                     ) 
                                                else if beats c1 c2 then Just ( Constraints.mustBeSignedBy (gFirst game)                            <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)                     
                                                     , State Finished mempty
                                                     )
                                                else Nothing
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce c1) _ = do  
    sha2_256 (nonce `concatenate` if c1 == Rock then bsRock' else if c1 == Paper then bsPaper' else bsScissors') == bs
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            ...
            GameDatum _ (Just c') | c == c' -> do
                logInfo @String "second player played and it was a draw"
                void $ mapError' $ runStep client $ Reveal (fpNonce fp) (fpChoice fp)
                logInfo @String "first player revealed and returned the bets"
    ...
    case m of
        ...
        Just ((o, _), _) -> case tyTxOutData o of
            GameDatum _ Nothing -> do
                ...
                m' <- mapError' $ getOnChainState client
                case m' of
                    Nothing -> logInfo @String "first player revealed"
                    Just _  -> do
